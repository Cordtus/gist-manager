	...
	useEffect(() => {
		...
		setCustomColors({
			primary:
				...
				defaultColors[theme === 'custom' ? 'light' : theme].primary,
			primaryLight:
				rootStyles.getPropertyValue('--color-primary-light').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].primaryLight,
			primaryDark:
				rootStyles.getPropertyValue('--color-primary-dark').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].primaryDark,
			background:
				rootStyles.getPropertyValue('--color-bg').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].background,
			surface:
				rootStyles.getPropertyValue('--color-surface').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].surface,
			surfaceSecondary:
				rootStyles.getPropertyValue('--color-surface-secondary').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].surfaceSecondary,
			textPrimary:
				rootStyles.getPropertyValue('--color-text-primary').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].textPrimary,
			textSecondary:
				rootStyles.getPropertyValue('--color-text-secondary').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].textSecondary,
			border:
				rootStyles.getPropertyValue('--color-border').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].border,
			success:
				rootStyles.getPropertyValue('--color-success').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].success,
			warning:
				rootStyles.getPropertyValue('--color-warning').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].warning,
			danger:
				rootStyles.getPropertyValue('--color-danger').trim() ||
				defaultColors[theme === 'custom' ? 'light' : theme].danger,
		});
	}, [theme, defaultColors]);
		...
		},
		{
			...
			colors: [
				...
				{ key: 'surface', label: 'Card Surface', example: 'Cards, modals, dropdowns' },
				{
					...
					example: 'Nested cards, code blocks',
				},
			],
		},
		{
			...
			<div className="space-y-6">
				<div className="flex items-center justify-between">
					<h3 className="text-lg font-semibold text-primary">
						{theme === 'custom'
							? 'Custom Theme Editor'
							: `Edit ${theme.charAt(0).toUpperCase() + theme.slice(1)} Theme`}
					</h3>
							...
							{group.colors.map(({ key, label, example }) => (
								<div key={key} className="flex items-center gap-3">
									<div className="flex-1">
										<label htmlFor={`color-${key}`} className="text-sm font-medium text-primary">
											{label}
										</label>
				...
				<button type="button" onClick={applyColors} className="button primary w-full">
