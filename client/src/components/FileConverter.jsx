	...
	const handleConvert = () => {
		...
		try {
			...
			// Default fallback for unsupported conversions
				...
				throw new Error(
					`Conversion from ${FORMATS[inputFormat].name} to ${FORMATS[outputFormat].name} is not supported`,
				);
			}
		...
		<div className="p-6 bg-surface rounded shadow-md space-y-6">
			<div>
				...
				<p className="text-sm text-secondary mt-1">
					Convert between Markdown, HTML, JSON, and Plain Text formats
				</p>
			</div>
					...
					<div className="flex gap-4 mb-2 items-end">
						...
						<div className="w-[200px]">
							<label
								...
								className="block text-sm font-medium text-secondary mb-1"
								title="The name for the downloaded file"
							>
								Output Filename
							</label>
							<input
								...
								{history.map(({ id, timestamp, inputFormat, outputFormat, inSize, outSize }) => (
									<tr key={id}>
										...
										<td className="px-4 py-2 text-sm">
											{FORMATS[inputFormat].name} → {FORMATS[outputFormat].name}
										</td>
										<td className="px-4 py-2 text-sm">
											{inSize}→{outSize}{' '}
											<span
