...
import { FaHome, FaListAlt, FaPlus, FaExchangeAlt, FaUsers, FaUser, FaPalette, FaPaintBrush } from 'react-icons/fa';
    ...
    <aside className="flex flex-col bg-gray-800 text-white w-64 h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-1">
        ...
        <div className="pt-4 mt-4 border-t border-gray-700">
          <h3 className="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Theme Tools</h3>
          <Link
            to="/theme-sandbox"
            className={`flex items-center px-4 py-2 rounded-lg transition-colors duration-200
              ${isActive('/theme-sandbox')
                ? 'bg-gray-700'
                : 'hover:bg-gray-700'}
            `}
          >
            <FaPalette className="mr-3" />
            <span>Theme Sandbox</span>
          </Link>
          <Link
            to="/theme-colors"
            className={`flex items-center px-4 py-2 rounded-lg transition-colors duration-200
