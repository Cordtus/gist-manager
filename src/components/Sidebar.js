...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-gray-800 ${theme === 'dark' ? 'bg-gray-900' : ''} text-white w-64 space-y-6 py-7 px-2 absolute inset-y-0 left-0 transform -translate-x-full md:relative md:translate-x-0 transition duration-200 ease-in-out`}>
      <div className="px-4">
      ...
      <nav className="space-y-1">
        <Link 
          ...
          className={`block py-2.5 px-4 rounded transition duration-200 ${
            isActive('/') 
              ? `bg-gray-700 ${theme === 'dark' ? 'bg-gray-800' : ''} text-white` 
              : `hover:bg-gray-700 hover:text-white ${theme === 'dark' ? 'hover:bg-gray-800' : ''}`
          }`}
        >
      ...
      <div className={`absolute bottom-0 left-0 w-full p-4 bg-gray-900 ${theme === 'dark' ? 'bg-black' : ''} text-center`}>
        {user ? (
          <>
